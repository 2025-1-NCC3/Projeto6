  ...
  return texto
    ...
    .map(char => {
      ...
      if (codigo >= 65 && codigo <= 90) {
        return String.fromCharCode(((codigo - 65 + deslocamento) % 26) + 65); // Maiúsculas
      } else if (codigo >= 97 && codigo <= 122) {
        return String.fromCharCode(((codigo - 97 + deslocamento) % 26) + 97); // Minúsculas
      } else {
        return char; // Outros caracteres (ex: números, @, etc.)
      }
    })
    .join('');
}
...
app.post("/regp", function (req, res) {
  ...
  var NomeCrip = cifraDeCesar(Nome, deslocamento);
  var CPFcrip = cifraDeCesar(CPF, deslocamento);
  var EmailCrip = cifraDeCesar(Email, deslocamento);
  // Senha pode ser mantida simples ou depois trocamos para hash

  var sql =
    "INSERT INTO Passageiro (Nome, Senha, CPF, Email) VALUES (?, ?, ?, ?)";

  db.run(sql, [NomeCrip, Senha, CPFcrip, EmailCrip], function (err) {
    if (err) {
...
app.post("/regm", function (req, res) {
  ...
  var EmailCrip = cifraDeCesar(Email, deslocamento);
  var CPFcrip = cifraDeCesar(CPF, deslocamento);
  var CelularCrip = cifraDeCesar(Celular, deslocamento);
  ...
  db.run(
    sql,
    [NomeCrip, Senha, EmailCrip, CPFcrip, CNH, Placa, Marca, Modelo, CelularCrip],
    function (err) {
      if (err) {
