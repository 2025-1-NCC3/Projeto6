...
app.post("/regp", async (req, res) => {
  ...
  try {
    ...
    const novoPassageiro = {
      ...
      CPF: CPF,
      Email: Email, // Salva o Email em texto plano
    ...
    const existingUser = await passageirosCollection.findOne({ Email: Email });
    if (existingUser) {
...
app.post("/regm", async (req, res) => {
  ...
  try {
    ...
    // Verifica se o email já existe
    const existingUser = await motoristasCollection.findOne({ Email: Email });
    if (existingUser) {
      return res.status(409).json({ error: "Email já cadastrado." });
    }
...
app.post("/loginp", async (req, res) => {
  ...
  try {
    ...
    // Busca o passageiro pelo email em texto plano
    const passageiro = await passageirosCollection.findOne({ Email: Email });
...
app.post("/loginm", async (req, res) => {
  ...
  try {
    ...
    // Busca o motorista pelo email em texto plano
    const motorista = await motoristasCollection.findOne({ Email: Email });
